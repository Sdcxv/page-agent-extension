...
export function normalizeResponse(response: any): any {
    ...
    if (toolCall?.function?.arguments) {
        ...
        if (toolCall.function.name && toolCall.function.name !== 'AgentOutput') {
            console.log(`[normalizeResponse] #1: fixing tool_call`)
        ...
        if (message.content) {
            ...
            if (jsonInContent) {
                ...
                if (resolvedArguments?.name === 'AgentOutput') {
                    console.log(`[normalizeResponse] #2: fixing tool_call`)
                ...
                if (resolvedArguments?.type === 'function') {
                    console.log(`[normalizeResponse] #3: fixing tool_call`)
                ...
                if (
                    ...
                    console.log(`[normalizeResponse] #4: fixing tool_call`)
                    resolvedArguments = { action: safeJsonParse(resolvedArguments) }
                }
            } else {
    ...
    if (!resolvedArguments.action) {
        console.log(`[normalizeResponse] #5: fixing tool_call`)
